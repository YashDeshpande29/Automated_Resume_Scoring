...
# sql connector
connection = pymysql.connect(host='localhost',user='root',password='viit@2026',db='cv')
    ...
    ###### CODE FOR ADMIN SIDE (ADMIN) ######
        ...
        if st.button('Login'):
            ...
            if ad_user == 'admin' and ad_password == 'admin@resume-analyzer':
                ...
                st.success("Welcome Yash ! Total %d " % values + " User's Have Used Our Tool : )")                
                ...
                # **Corrected Pie Charts using `plot_data` DataFrame**
                plot_data['count'] = plot_data['Predicted_Field'].value_counts()

                field_counts = plot_data.groupby('Predicted_Field').size().reset_index(name='count')
                fig = px.pie(field_counts, values='count', names='Predicted_Field', title='Predicted Field according to the Skills 👽', color_discrete_sequence=px.colors.sequential.Aggrnyl_r)
                st.plotly_chart(fig)

                # # Pie chart for User's👨‍💻 Experienced Level
                # st.subheader("**Pie-Chart for User's Experienced Level**")
                # fig = px.pie(plot_data, values='User_Level', names='User_Level', title="Pie-Chart 📈 for User's 👨‍💻 Experienced Level", color_discrete_sequence=px.colors.sequential.RdBu)
                # st.plotly_chart(fig)

                # Pie chart for Resume Score
                st.subheader("**Pie-Chart for Resume Score**")
                fig = px.pie(plot_data, values='resume_score', names='resume_score', title='From 1 to 100 💯', color_discrete_sequence=px.colors.sequential.Agsunset)
                st.plotly_chart(fig)
                ...
                # fig = px.pie(plot_data, values='IP_add', names='IP_add', title='Usage Based On IP Address 👥', color_discrete_sequence=px.colors.sequential.matter_r)
                # st.plotly_chart(fig)

                # # Pie chart for City
                # st.subheader("**Pie-Chart for City**")
                # fig = px.pie(plot_data, values='City', names='City', title='Usage Based On City 🌆', color_discrete_sequence=px.colors.sequential.Jet)
                # st.plotly_chart(fig)

                # # Pie chart for State
                # st.subheader("**Pie-Chart for State**")
                # fig = px.pie(plot_data, values='State', names='State', title='Usage Based on State 🚉', color_discrete_sequence=px.colors.sequential.PuBu_r)
                # st.plotly_chart(fig)

                # # Pie chart for Country
                # st.subheader("**Pie-Chart for Country**")
                # fig = px.pie(plot_data, values='Country', names='Country', title='Usage Based on Country 🌏', color_discrete_sequence=px.colors.sequential.Purpor_r)
                # st.plotly_chart(fig)
